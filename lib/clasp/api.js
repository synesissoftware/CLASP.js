...
class Arguments {
	...
	constructor(argv, aliases, options = {}) {
...
function _select_alias(item, aliases) {
...
function _select_aliases(item, aliases) {

	var select_alias = _select_alias(item, aliases);
	...
	if (n > 1) {
		...
		if (m) {
			...
			for (var i = 0; i != gr1.length; ++i) {
				...
				select_alias	=	_select_alias(name, aliases);
				...
				if (select_alias) {
					...
					if (m2) {
						...
						var name2				=	m2[1] + m2[2];
						var select_option_alias	=	_select_alias(name2, aliases);
...
function _parse(argv, aliases, options = {}) {
	...
	for (var index = 0; index + 1 < argv.length; ++index) {
		...
		if (m) {
			...
			if (given_name != arg) {
				...
				var sel_aliases	=	_select_aliases(arg, aliases);
					...
					if (Array.isArray(sel_aliases)) {
						...
						for (var j = 0; j < sel_aliases.length; ++j) {
							...
							if (false) {
								...
								var flag	=	new FlagArgument(arg, index, arg, a.name, a, hyphens.length, given_label, a.extras);
								...
								var	option	=	new OptionArgument(arg, index, arg, soa.name, fa, hyphens.length, given_label, v, extras);
